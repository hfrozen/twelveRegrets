 ...
 * @file	CPump.h
 * @brief	태그-메시지 전송
 * @details	큐를 사용하여 클래스들 사이로 태그를 전송하고 태그를 받은 클라스는 해당 태그에 해당하는 함수를 실행한다.
 * 			그냥 해당 클래스의 함수를 콜하여도 되겠지만 큐에 메시지를 전달한다는 것은 OS에 CPU를 돌려준다는 점에서 의미가 더 깊다.
 * @author	Che-follo
 * @data	2015-11-XX
 */
#pragma once
...
/**
 * 아래의 선언들은 MS Visual C MFC에서 카피한 것 들이다.
 */
struct TAGENVELOPE;
struct TAGTABLE {
