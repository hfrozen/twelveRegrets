...
void CFio::SetChar(int x, BYTE ch)
{
	...
	BYTE buf[5];
	for (int n = 0; n < 5; n ++)	buf[n] = c_cFont[ch][n];
	SetChar(x, buf);
}

void CFio::SetDots(int x, int iSize)
{
	if (iSize > 35)	iSize = 35;

	BYTE buf[5];
	memset(buf, 0, sizeof(buf));
	for (int n = 0; n < iSize; n ++)	buf[n % 5] |= (1 << (6 - n / 5));
	SetChar(x, buf);
	printf("SetDots %d\n", iSize);
}
