...
bool CFsc::Clean(int iCh)
{
	...
	for ( ; (wFlag = (SBREG(iCh))) & (1 << (iCh & 3)); n ++) {
		WORD leng = SCREG(iCh, COMRXLN);	// & (MAX_ROADBUFW - 1);
		if (leng > 0) {
...
void CFsc::Receive(int iCh)
{
	...
	int iBlock = iCh >> 2;
	c_bc[iBlock].mtx.Lock();
	WORD wState = SCREG(iCh, COMSTAT);
	if ((wState & COMSTAT_RXES) == 0) {
		Pile("FSC>RECV>ERR:%d s%04X!\n", iCh, wState);
		c_bc[iBlock].mtx.Unlock();
		return;
	}

	memset(c_ic[iCh].wRxBuf, 0, sizeof(WORD) * MAX_ROADBUFW);
	WORD wLeng = 0;
	if (wState & (1 << COMSTAT_RXBE)) {
		WORD len = SCREG(iCh, COMRXLN);		// byte length
		if (len == 0) {
			Pile("FSC>RECV>ERR:%d s %04X w%d!\n", iCh, wState, len);
			c_bc[iBlock].mtx.Unlock();
			return;
		}
		if (len >= (MAX_ROADBUFW * sizeof(WORD))) {
			Pile("FSC>RECV>ERR:%d w%d!\n", iCh, len);
			len = (MAX_ROADBUFW - 1) * sizeof(WORD);
		}
		wLeng = len;
		if (len & 1) ++ len;
		len >>= 1;
		...
		if (c_ic[iCh].wMode & (1 << COMCTRL_AMEN)) {
			c_ic[iCh].wRxBuf[0] = SCREG(iCh, COMRXBF);
			w = 1;
		}
		for ( ; w < len; w ++)	c_ic[iCh].wRxBuf[w] = SCREG(iCh, COMRXBF);
	}
	++ c_ic[iCh].wRxSeq;
	c_ic[iCh].wRxBuf[MAX_ROADBUFW - 3] = wState;
	c_ic[iCh].wRxBuf[MAX_ROADBUFW - 2] = wLeng;
	c_ic[iCh].wRxBuf[MAX_ROADBUFW - 3] = c_ic[iCh].wRxSeq;

	CPump* pPump = (CPump*)c_ic[iCh].pBase;
	PRECVINTR pRecv = c_ic[iCh].pRecvIntr;
	if (c_ic[iCh].bEnIntr && pRecv != NULL && pPump != NULL)
		(pPump->*pRecv)((BYTE*)c_ic[iCh].wRxBuf, wLeng, wState & COMSTAT_RXERROR);
	else {
		Clean(iCh);
		Pile("FSC>INTR>ERR:ch%d null fumnc. or disable!\n", iCh);
	}
	c_bc[iBlock].mtx.Unlock();
}
...
bool CFsc::Appear(int iCh, WORD wAddr, enENCMODE mode, DWORD dwBps, bool bMatch, int iFlagLength, PRECVINTR pRecvIntr, CPump* pBase)
{
	CHECK_ROADCH(iCh);
...
bool CFsc::Shoot(int iCh, PVOID p, WORD wLength)
{
	...
	if (dw >= 100) {
		if (iCh == 0 || iCh == 1) {
			...
			if (pFio != NULL)	pFio->Lamp(iCh != 0 ? CFio::EPOS_SHOTB : CFio::EPOS_SHOTA);
		}
